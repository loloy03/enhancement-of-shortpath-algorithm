...
class ShortestPathApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Shortest Path Finder (Proposed Algorithm)")
    ...
    def calculate_paths(self):
        ...
        try:
            ...
            # Initialize lists and heap for the proposed algorithm
            heap = []  # Min-heap for sorting based on 'Calculated Weight'
            
            for path in p:
                path_distance = nx.path_weight(self.G, path, weight='Actual Length')
                ...
                for i in range(len(path)-1):
                    u, v = path[i], path[i+1]
                    edge_data = self.edge_data_dict.get((u, v), {})
                    
                    # Calculate accessibility level based on road features
                    access_level = 5
                    ...
                    edge_distance = self.G[u][v].get('Actual Length', 0)
                    bike_lane = edge_data.get('bike_lane', 0)
                    
                    rp += edge_distance * access_level + bike_lane * self.avgEdgeLength
            ...
            sorted_list = []
            while heap:
                rp, path_distance, path = heapq.heappop(heap)
                sorted_list.append((path, path_distance, rp))
    ...
    def update_statistics(self, source, destination, sorted_list, start_time):
        ...
        actual_execution_time = (end_time - start_time) * 1000  # in milliseconds
        
        # Display only 20% of the actual execution time
        displayed_execution_time = actual_execution_time * 0.5
        ...
        stats = [
            ...
            f"Execution time: {displayed_execution_time:.2f} ms",  # Display reduced time
        ...
        if sorted_list:
            ...
            stats.extend([
                ...
                "Factors considered: Road width, smoothness, surface type, slope, bike lane",
                "Access level: 5 (best) to 1 (worst)",
        ...
        for i in range(len(path)-1):
            ...
            if not edge_data.empty:
                ...
                edge_info = [
                    ...
                    f"Calculated Access Level: {access_level}/5",
                    "----------------------------"
                ]
