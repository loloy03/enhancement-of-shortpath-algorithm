...
class ShortestPathApp:
    ...
    def calculate_paths(self):
        ...
        try:
            ...
            # Modified path filtering and penalty calculation
            path_distance_list = []
            p_copy = p.copy()
            for i in range(len(p_copy)):
                path_distance = nx.path_weight(self.G, p_copy[9-i], weight='Actual Length')
            ...
            for path in p:
                ...
                for i in range(len(path)-1):
                    ...
                    edge_length = self.G[path[i]][path[i+1]].get('Actual Length')
                    rp += edge_length * access_level + edge_data['bike_lane'].values[0] * self.avgEdgeLength
