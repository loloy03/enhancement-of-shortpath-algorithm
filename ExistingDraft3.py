...
class ShortestPathApp:
    def __init__(self, root):
        ...
        # Create edge data dictionary for faster lookup
        self.edge_data_dict = self.df.set_index(['Source Node', 'Destination Node']).to_dict(orient="index")
        ...
        self.G = nx.from_pandas_edgelist(
            ...
            edge_attr=['Actual Length', 'road_width', 'smoothness', 'surface', 'slope', 'bike_lane'],
    ...
    def calculate_paths(self):
        ...
        try:
            ...
            # Modified path filtering from calculate_existing_algo
            path_distance_list = []
            p_copy = p.copy()
            for i in range(len(p_copy)):
                path_distance = nx.path_weight(self.G, p_copy[len(p_copy) - 1 - i], weight='Actual Length')
            ...
            # Compute penalty using the modified logic from calculate_existing_algo
            rp_list = []
            for path in p:
                rp = 0
                for u, v in zip(path, path[1:]):
                    if (u, v) in self.edge_data_dict:
                        edge_data = self.edge_data_dict[(u, v)]
                        access_level = 5
                        if edge_data['road_width'] > 3 and edge_data['smoothness'] in ["excellent", "good"] and edge_data['surface'] in ["asphalt", "concrete"] and edge_data['slope'] in ["flat", "gentle"]:
                            access_level = 1
                        rp += self.G[u][v]['Actual Length'] * access_level + edge_data['bike_lane'] * self.avgEdgeLength
                rp_list.append(rp)
            
            # Create zipped list and sort by penalty (rp)
    ...
    def display_path_details(self, path):
        ...
        for i in range(len(path)-1):
            ...
            # Get edge data using the dictionary for faster lookup
            if (source, dest) in self.edge_data_dict:
                edge_data = self.edge_data_dict[(source, dest)]
                
                # Calculate accessibility level based on attributes
                access_level = 5  # Start with worst level
                if (edge_data['road_width'] > 3 and 
                    edge_data['smoothness'] in ["excellent", "good"] and 
                    edge_data['surface'] in ["asphalt", "concrete"] and 
                    edge_data['slope'] in ["flat", "gentle"]):
                    access_level = 1  # Best level
                
                edge_info = [
                    f"Edge: {source} → {dest}",
                    f"Actual Length: {edge_data['Actual Length']}",
                    f"Road Width: {edge_data['road_width']}",
                    f"Smoothness: {edge_data['smoothness']}",
                    f"Surface: {edge_data['surface']}",
                    f"Slope: {edge_data['slope']}",
                    f"Bike Lane: {'Yes' if edge_data['bike_lane'] == 1 else 'No'}",
                    f"Calculated Accessibility Level: {access_level}",
                    "----------------------------"
                ]
