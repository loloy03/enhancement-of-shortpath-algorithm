...
def calculate_existing_algo(df, G, p, apLengthThreshold, avgEdgeLength, edge_data_dict):
    ...
    loop_time_1 = (end_time_first_loop - start_time_first_loop) * 1000
    # tracemalloc.stop()
    ...
    loop_time_2 = (end_time_second_loop - start_time_second_loop) * 1000
    existing_time = loop_time_1 + loop_time_2
    # tracemalloc.stop()
...
def calculate_propose_algo(df, G, p, apLengthThreshold, avgEdgeLength, edge_data_dict):
    ...
    propose_time = (end_time_combined_loop - start_time_combined_loop) * 1000
    # tracemalloc.stop()
...
print(f"Execution Time : {((existing_time - propose_time) / existing_time) * 100:.2f}%")
print(f"Memory Usage : {((current_existing - current_propose) / current_existing) * 100:.2f}%")